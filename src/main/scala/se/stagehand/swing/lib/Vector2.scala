 ...
 */
object Vector2 {
  implicit def PointToVec2(p:Point) = Vector2(p.x,p.y)
  implicit def Vec2ToPoint(v:Vector2) = new Point(v.x,v.y)
  implicit def DimToVec2(d:Dimension) = Vector2(d.width,d.height)
  implicit def Vec2ToDim(v:Vector2) = new Dimension(v.x,v.y)
